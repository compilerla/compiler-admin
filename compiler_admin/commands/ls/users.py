...
def google(format: int = Format.BASIC, inactive: bool = False, **kwargs):
    """Use GAM to print the users in the Google Workspace."""
    output = get_users(inactive=inactive, format=format, **kwargs)
    click.echo(output)


def toggl(format: int = Format.BASIC, inactive: bool = False, **kwargs):
    ...
    users = api.get_organization_users(inactive=inactive, **kwargs)
...
def users(system: str, inactive: bool, format_key: str, **kwargs):
    """List users in the Compiler workspace."""
    format = FORMATS.get(format_key)

    ls_system = USER_SYSTEMS.get(system)
    ls_system(format=format, inactive=inactive, **kwargs)
